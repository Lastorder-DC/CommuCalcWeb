...
export default function LoginPage() {
  ...
  const handleMastodonLogin = async (serverIndex: number) => {
    setError('');
    setLoading(true);
    try {
      const { authorizeUrl } = await getMastodonLoginUrl(serverIndex);
      window.location.href = authorizeUrl;
    } catch (err) {
      ...
      <div className="col-md-6 col-lg-4">
        ...
        {hasOAuthLogin && (
          <div className="mt-3">
            ...
            {mastodonLoginEnabled && mastodonServers.map((server) => (
              <button
                key={server.index}
                type="button"
